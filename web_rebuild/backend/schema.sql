...
CREATE TABLE IF NOT EXISTS wx_article_detail (
  ...
  article_url VARCHAR(2048) NOT NULL COMMENT '文章链接',
  url_hash BINARY(16) GENERATED ALWAYS AS (UNHEX(MD5(article_url))) STORED COMMENT 'URL哈希',
  post_time BIGINT NULL COMMENT '发文时间戳',
  wxid VARCHAR(64) NULL COMMENT '微信公众号ID',
  author VARCHAR(128) NULL COMMENT '作者',
  html MEDIUMTEXT NULL COMMENT '文章正文HTML',
  fetched_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '抓取时间',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_url (url_hash),
